...
;			EntityParent dp\cam,fr\Forest_Pivot
		...
		Case "gatea"
			...
			d2 = CreateDoor(r\zone, r\x-1440*RoomScale, r\y-480.0*RoomScale, r\z + 2328.0 * RoomScale, 0, r, False, False, 3)
			...
			d2 = CreateDoor(r\zone, r\x-1440*RoomScale, r\y-480.0*RoomScale, r\z + 4352.0 * RoomScale, 0, r, False, False, 3)
			If SelectedEnding = "A2" Then 
				d2\AutoClose = False : d2\open = True : d2\locked = True	
			Else
				d2\AutoClose = False : d2\open = False : d2\locked = False
		...
		Case "exit1"
			...
			;PositionEntity(r\Objects[18], r\x+3727.0*RoomScale, 10.0*RoomScale, r\z+6623.0*RoomScale, True)
		...
		Case "checkpoint1"
			...
			PositionEntity(r\RoomDoors[0]\buttons[1], r\x - 152.0 * RoomScale, EntityY(r\RoomDoors[0]\buttons[1],True), r\z + 120.0 * RoomScale, True)
			...
			PositionEntity (r\Objects[2], r\x - 152.0*RoomScale, 384.0*RoomScale, r\z+130.0*RoomScale, True)
		...
		Case "room2shaft"
			...
			d = CreateDoor(r\zone, r\x + 256.0 * RoomScale, r\y, r\z + 744.0 * RoomScale, 90, r, False, False, 3)
			d\AutoClose = False : d\open = False
		...
		Case "room2nuke"
			;[Block]
			;"tuulikaapin" ovi
			d = CreateDoor(r\zone, r\x + 576.0 * RoomScale, 0.0, r\z + 152.0 * RoomScale, 90, r, False, False, 4)
			d\AutoClose = False : d\open = False
			...
			d = CreateDoor(r\zone, r\x - 544.0 * RoomScale, 1504.0*RoomScale, r\z + 738.0 * RoomScale, 90, r, False, False, 4)
			d\AutoClose = False : d\open = False			
...
;			
		...
		Case "room049"
			...
			r\RoomDoors[0] = CreateDoor(r\zone, r\x + 330.0 * RoomScale, 0.0, r\z + 656.0 * RoomScale, 90, r, True, 4)
			r\RoomDoors[0]\AutoClose = False : r\RoomDoors[0]\open = True
...
;			PositionEntity(r\Objects[6], r\x - 848*RoomScale, 0.5, r\z-576*RoomScale, True)
		...
		Case "coffin"
			;[Block]
			d = CreateDoor(r\zone, r\x, 0, r\z + 1596.0 * RoomScale, 0, r, False, True, 2)
			d\AutoClose = False : d\open = False
			PositionEntity(d\buttons[0], r\x - 425.0 * RoomScale, 0.7, r\z + 1859.0 * RoomScale, True)
			RotateEntity(d\buttons[0], 0, 180, 0)
			
			d.Doors = CreateDoor(r\zone, r\x + 0.0 * RoomScale, 0.0, r\z + 1023.0 * RoomScale, 0, r, False, False, 3)
			...
			sc.SecurityCams = CreateSecurityCam(r\x - 325.0 * RoomScale, r\y + 704 * RoomScale, r\z + 2315.0 * RoomScale, r, True)
			sc\angle = 45 + 180
			sc\turn = 45
			...
			PositionEntity(sc\ScrObj, r\x - 800 * RoomScale, 296.0 * RoomScale, r\z + 1703.0 * RoomScale)
			EntityParent(sc\ScrObj, r\obj)
			...
			For i% = 0 To 1
				...
				PositionEntity (r\Objects[2 + i], r\x - 795.0 * RoomScale, r\y + 195.0 * RoomScale, r\z + 1715 * RoomScale, True)
			...
			it = CreateItem("Document SCP-895", "paper", r\x - 688.0 * RoomScale, r\y + 133.0 * RoomScale, r\z + 1743.0 * RoomScale)
			EntityParent(it\collider, r\obj)
			
			it = CreateItem("Level 3 Key Card", "key3", r\x + 240.0 * RoomScale, r\y -1456.0 * RoomScale, r\z + 4145.0 * RoomScale)
			EntityParent(it\collider, r\obj)
		...
		Case "914"
			...
			;PositionEntity (d\buttons[0], r\x - 496.0 * RoomScale, 0.7, r\z - 272.0 * RoomScale, True)
			;TurnEntity(d\buttons[0], 0, 90, 0)
			r\RoomDoors[2] = CreateDoor(r\zone, r\x - 4338.0 * RoomScale, r\y - 2495.0 * RoomScale, r\z - 3097.0 * RoomScale, 0, r, False, False, 2)
			...
			d.Doors = CreateDoor(r\zone, r\x - 4320.0 * RoomScale,r\y - 2495.0 * RoomScale, r\z + 5200.0 * RoomScale, 0, r, False, False)
			PositionEntity(d\buttons[0], EntityX(d\buttons[0],True), EntityY(d\buttons[0],True), EntityZ(d\buttons[0],True), True)
			PositionEntity(d\buttons[1], EntityX(d\buttons[1],True), EntityY(d\buttons[1],True), EntityZ(d\buttons[1],True), True)
			
			; Locked doors
			
			d.Doors = CreateDoor(r\zone, r\x + 2655.0 * RoomScale, r\y - 2495.0 * RoomScale, r\z + 5166.0 * RoomScale, 0, r, False, False)
			PositionEntity(d\buttons[0], EntityX(d\buttons[0],True), EntityY(d\buttons[0],True), EntityZ(d\buttons[0],True), True)
			PositionEntity(d\buttons[1], EntityX(d\buttons[1],True), EntityY(d\buttons[1],True), EntityZ(d\buttons[1],True), True)
			d.Doors\locked = True
			
			d.Doors = CreateDoor(r\zone, r\x + 2648.0 * RoomScale, r\y - 2495.0 * RoomScale, r\z - 3085.0 * RoomScale, 0, r, False, False)
			PositionEntity(d\buttons[0], EntityX(d\buttons[0],True), EntityY(d\buttons[0],True), EntityZ(d\buttons[0],True), True)
			PositionEntity(d\buttons[1], EntityX(d\buttons[1],True), EntityY(d\buttons[1],True), EntityZ(d\buttons[1],True), True)
			d.Doors\locked = True
			...
			d.Doors = CreateDoor(r\zone, r\x + 2639.0 * RoomScale, r\y - 2495.0 * RoomScale, r\z + 1550.0 * RoomScale, 0, r, False, False)
			PositionEntity(d\buttons[0], EntityX(d\buttons[0],True), EntityY(d\buttons[0],True), EntityZ(d\buttons[0],True), True)
			PositionEntity(d\buttons[1], EntityX(d\buttons[1],True), EntityY(d\buttons[1],True), EntityZ(d\buttons[1],True), True)
			
			d.Doors = CreateDoor(r\zone, r\x - 4338.0 * RoomScale, r\y - 2495.0 * RoomScale, r\z + 542.0 * RoomScale, 0, r, False, False)
			PositionEntity(d\buttons[0], EntityX(d\buttons[0],True), EntityY(d\buttons[0],True), EntityZ(d\buttons[0],True), True)
			PositionEntity(d\buttons[1], EntityX(d\buttons[1],True), EntityY(d\buttons[1],True), EntityZ(d\buttons[1],True), True)
			
			d.Doors = CreateDoor(r\zone, r\x - 3310.0 * RoomScale, r\y - 2495.0 * RoomScale, r\z - 4101.0 * RoomScale, 90, r, False, False)
			PositionEntity(d\buttons[0], EntityX(d\buttons[0],True), EntityY(d\buttons[0],True), EntityZ(d\buttons[0],True), True)
			PositionEntity(d\buttons[1], EntityX(d\buttons[1],True), EntityY(d\buttons[1],True), EntityZ(d\buttons[1],True), True)
			
			; 914 Key and Knob
			
			PositionEntity(r\Objects[0], r\x - 5722.0 * RoomScale, r\y - 2305.0 * RoomScale, r\z - 4367.0 * RoomScale)
			PositionEntity(r\Objects[1], r\x - 5722.0 * RoomScale, r\y - 2265.0 * RoomScale, r\z - 4367.0 * RoomScale)
			EntityParent(r\Objects[0], r\obj)
			EntityParent(r\Objects[1], r\obj)
			
			; 914 Doors
			
			d = CreateDoor(r\zone, r\x - 6347.0 * RoomScale, r\y - 2495.0 * RoomScale, r\z - 4211.0 * RoomScale, 180, r, True)
			FreeEntity (d\obj2) : d\obj2 = 0
			FreeEntity (d\buttons[0]) : d\buttons[0] = 0
			FreeEntity (d\buttons[1]) : d\buttons[1] = 0
			d\dir = 4
			...
			d = CreateDoor(r\zone, r\x - 4909.0 * RoomScale, r\y - 2495.0 * RoomScale, r\z - 4219.0 * RoomScale, 180, r, True)
			FreeEntity (d\obj2) : d\obj2 = 0	
			FreeEntity (d\buttons[0]) : d\buttons[0] = 0
			FreeEntity (d\buttons[1]) : d\buttons[1] = 0
			d\dir = 4
			...
			r\Objects[2] = CreatePivot()
			r\Objects[3] = CreatePivot()
			PositionEntity(r\Objects[2], r\x - 6429.0 * RoomScale, r\y - 2400.0 * RoomScale, r\z - 4093.0 * RoomScale)
			PositionEntity(r\Objects[3], r\x - 4989.0 * RoomScale, r\y - 2200.0 * RoomScale, r\z - 4087.0 * RoomScale)
			EntityParent(r\Objects[2], r\obj)
			EntityParent(r\Objects[3], r\obj)
			
			;elevator doors
			r\RoomDoors[3] = CreateDoor(r\zone, r\x - 447.0 * RoomScale, r\y, r\z, 90, r, False, 3)
			r\RoomDoors[3]\AutoClose = False : r\RoomDoors[3]\open = True
			
			r\RoomDoors[4] = CreateDoor(r\zone, r\x + 2360.0 * RoomScale, r\y - 2495.0 * RoomScale, r\z + 655.0 * RoomScale, 90, r, False, 3) ;799 -287
            r\RoomDoors[4]\AutoClose = False : r\RoomDoors[0]\open = True
            PositionEntity (r\RoomDoors[4]\buttons[1], r\x + 2325.0 * RoomScale, r\y - 2303 * RoomScale, r\z + 494.0 * RoomScale, True)
            PositionEntity (r\RoomDoors[4]\buttons[0], r\x + 2425.0 * RoomScale, r\y - 2303 * RoomScale, r\z + 810.0 * RoomScale, True)
			
			;elevator pivots
			r\Objects[8] = CreatePivot()
			PositionEntity(r\Objects[8], r\x - 752.0 * RoomScale, r\y + 66.0 * RoomScale, r\z)
            EntityParent(r\Objects[8], r\obj)
			...
			it = CreateItem("First Aid Kit", "firstaid", r\x - 2857.0 * RoomScale, r\y - 2226.0 * RoomScale, r\z - 4110.0 * RoomScale)
			EntityParent(it\collider, r\obj) : RotateEntity(it\collider, 0, 90, 0)
		...
		Case "room2ccont"
			;[Block]
			d = CreateDoor(r\zone, r\x + 64.0 * RoomScale, 0.0, r\z + 368.0 * RoomScale, 180, r, False, False, 3)
			d\AutoClose = False : d\open = False
		...
		Case "room005"
			...
			d.Doors = CreateDoor(r\zone, r\x - 192.0 * RoomScale, 0.0, r\z - 688.0 * RoomScale, 0, r, False, False, 3)
			PositionEntity(d\buttons[0], EntityX(d\buttons[0],True), EntityY(d\buttons[0],True), EntityZ(d\buttons[0],True), True)
			PositionEntity(d\buttons[1], EntityX(d\buttons[1],True), EntityY(d\buttons[1],True), EntityZ(d\buttons[1],True), True)
			d.Doors\open = True 
		...
		Case "room1188"
            ...
            PositionEntity(d\buttons[0], EntityX(d\buttons[0],True), EntityY(d\buttons[0],True), EntityZ(d\buttons[0],True), True)
			PositionEntity(d\buttons[1], r\x + 713.0 * RoomScale, EntityY(d\buttons[1],True), r\z + 500 * RoomScale, True) 
			
			d.Doors = CreateDoor(r\zone, r\x + 831.0 * RoomScale, 0.0, r\z - 561.0 * RoomScale, 180, r, False, False)
            PositionEntity(d\buttons[0], EntityX(d\buttons[0],True), EntityY(d\buttons[0],True), EntityZ(d\buttons[0],True), True)
			PositionEntity(d\buttons[1], r\x + 713.0 * RoomScale, EntityY(d\buttons[1],True), r\z - 570 * RoomScale, True)
			d.Doors\open = True 
			
			d.Doors = CreateDoor(r\zone, r\x - 176.0 * RoomScale, 0.0, r\z + 703.0 * RoomScale, 90, r, False, False, 3)
			PositionEntity(d\buttons[0], EntityX(d\buttons[0],True), EntityY(d\buttons[0],True), EntityZ(d\buttons[0],True), True)
			PositionEntity(d\buttons[1], EntityX(d\buttons[1],True), EntityY(d\buttons[1],True), EntityZ(d\buttons[1],True), True)
			d.Doors\open = True 
		Case "room1warehouse"
		...
		Case "room2scps2"
			...
			d = CreateDoor(r\zone, r\x + 777.0*RoomScale, r\y, r\z + 671.0*RoomScale, 90, r, False, False, 3)
...
Function CreateMap()
	...
	SetRoom("room2wlobservation", ROOM2, Floor(0.3*Float(Room1Amount[0])),min_pos,max_pos)
	...
	SetRoom("room2floppa", ROOM2, min_pos+Floor(0.35*Float(Room2Amount[2])),min_pos,max_pos)
