...
Font1% = AALoadFont("GFX\font\TI\TI-83.ttf", Int(19 * (GraphicHeight / 1024.0)), 0,0,0)
Font2% = AALoadFont("GFX\font\TI\TI-83.ttf", Int(58 * (GraphicHeight / 1024.0)), 0,0,0)
...
Global RealisticBlinking% = False
Global RealisticStamina% = False
Global NVBlink%
		...
		If KeyHit(28) And ConsoleInput <> "" Then
			...
			Select Lower(StrTemp)
				Case "help"
					...
					Select Lower(StrTemp)
						...
						Case "spawn"
							...
							CreateConsoleMsg("008zombie / 049 / 049-2 / 096 / 106 / 173")
				...
				Case "infinitestamina","infstam"
					...
					EndIf
					;[End Block]
				Case "realisticblinking","realblink"
					;[Block]
					StrTemp$ = Lower(Right(ConsoleInput, Len(ConsoleInput) - Instr(ConsoleInput, " ")))
					
					Select StrTemp
						Case "on", "1", "true"
							RealisticBlinking% = True						
						Case "off", "0", "false"
							RealisticBlinking% = False
						Default
							RealisticBlinking% = Not RealisticBlinking%
					End Select
					
					If RealisticBlinking
						CreateConsoleMsg("REALISTIC BLINKING ON")
					Else
						CreateConsoleMsg("REALISTIC BLINKING OFF")	
					EndIf
					;[End Block]
				Case "realisticstamina","realstam"
					;[Block]
					StrTemp$ = Lower(Right(ConsoleInput, Len(ConsoleInput) - Instr(ConsoleInput, " ")))
					
					Select StrTemp
						Case "on", "1", "true"
							RealisticStamina% = True						
						Case "off", "0", "false"
							RealisticStamina% = False
						Default
							RealisticStamina% = Not RealisticStamina%
					End Select
					
					If RealisticStamina
						CreateConsoleMsg("REALISTIC STAMINA ON")
					Else
						CreateConsoleMsg("REALISTIC STAMINA OFF")	
					EndIf
					;[End Block]
				Case "realisticmeters"
					;[Block]
					StrTemp$ = Lower(Right(ConsoleInput, Len(ConsoleInput) - Instr(ConsoleInput, " ")))
					
					Select StrTemp
						Case "on", "1", "true"
						    If Not RealisticStamina% And RealisticBlinking% Then
							RealisticStamina% = True
							RealisticBlinking% = True
						EndIf						
						Case "off", "0", "false"
						    If RealisticStamina% And RealisticBlinking% Then
							RealisticStamina% = False
							RealisticBlinking% = False
						EndIf
						Default
					...
					If RealisticStamina% And RealisticBlinking%
						CreateConsoleMsg("REALISTIC METERS ON")
					Else
						CreateConsoleMsg("REALISTIC METERS OFF")	
					EndIf
					;[End Block]
				Case "asd2"
					;[Block]
...
Global DoorOBJ%, DoorFrameOBJ%, DoorFrameElevOBJ%
...
Repeat
	...
	Else
		...
		If RealisticBlinking% Then BlinkTimer = Min(150, BlinkTimer + (30-BlinkTimer)*0*FPSfactor)
		
		If RealisticStamina% Then Stamina = Min(50, Stamina + (30-Stamina)*0*FPSfactor)
		...
		Else
			...
			If EndingTimer < -1000 And EndingTimer > -2000
				...
				If AchievementsMenu=0 Then 
					...
					AAText x, y+20*MenuScale, "Unlockables earned: " + achievementsUnlocked+"/"+(MAXACHIEVEMENTS)
					...
					If DrawButton(x-145*MenuScale,y-200*MenuScale,390*MenuScale,60*MenuScale,"Unlockables", True) Then
						AchievementsMenu = 1
					EndIf
					
;					If DrawButton(x-145*MenuScale,y-100*MenuScale,390*MenuScale,60*MenuScale,"Escape", True) Then
;						NullGame()
;						StopStream_Strict(MusicCHN)
...
;					EndIf
					...
					If DrawButton(x-145*MenuScale,y-100*MenuScale,390*MenuScale,60*MenuScale,"Escape", True)
						ShouldPlay = 24
						NowPlaying = ShouldPlay
						For i=0 To 9
...
Function InitCredits()
	...
	CreditsFont% = LoadFont_Strict("GFX\font\TI\TI-83.ttf", Int(21 * (GraphicHeight / 1024.0)), 0,0,0)
	CreditsFont2% = LoadFont_Strict("GFX\font\TI\TI-83.ttf", Int(35 * (GraphicHeight / 1024.0)), 0,0,0)
...
Function MovePlayer()
	...
	If KeyHit(KEY_CROUCH) And Playable Then
	    PlaySound_Strict(CrouchSFX(Rand(0,5)))
	    Crouch = (Not Crouch)
	EndIf 
...
Function MouseLook()
	...
	If KillTimer >= 0 And FallTimer >=0 Then
	    ...
	    PlaySound_Strict("SFX\Horror\Death.ogg")
			...
			Select SelectedItem\itemtemplate\tempname
				...
				Case "navigator", "nav"
					...
					If PlayerRoom\RoomTemplate\Name$ = "pocketdimension" Or PlayerRoom\RoomTemplate\Name$ = "dimension1499" Then
						NavWorks% = False
					ElseIf PlayerRoom\RoomTemplate\Name$ = "room895" Then
					    NavWorks% = False
...
Function DrawMenu()
	...
	If MenuOpen Then
		...
		If AchievementsMenu > 0 Then
			AASetFont Font2
			AAText(x, y-(122-45)*MenuScale, "Unlockables",False,True)
			AASetFont Font1
		ElseIf OptionsMenu > 0 Then
			AASetFont Font2
			AAText(x, y-(122-45)*MenuScale, "Settings",False,True)
			AASetFont Font1
		ElseIf QuitMSG > 0 Then
			AASetFont Font2
			AAText(x, y-(122-45)*MenuScale, "YOUCANTESCAPE",False,True)
			AASetFont Font1
		...
		Else
			AASetFont Font2
			AAText(x, y-(122-45)*MenuScale, "Signal Lost",False,True)
			AASetFont Font1
		End If		
		...
		ElseIf AchievementsMenu <= 0 And OptionsMenu > 0 And QuitMSG <= 0 And KillTimer >= 0
			...
			If DrawButton(x-5*MenuScale,y,100*MenuScale,30*MenuScale,"Visuals",False) Then OptionsMenu = 1
			If DrawButton(x+105*MenuScale,y,100*MenuScale,30*MenuScale,"Sound",False) Then OptionsMenu = 2
			If DrawButton(x+215*MenuScale,y,100*MenuScale,30*MenuScale,"Ctrls+Binds",False) Then OptionsMenu = 3
			If DrawButton(x+325*MenuScale,y,100*MenuScale,30*MenuScale,"Misc",False) Then OptionsMenu = 4
			...
			Select OptionsMenu
				...
				Case 4 ;Advanced
					...
					If AATextEnable_Prev% <> AATextEnable
						...
						EndIf
						InitAAFont()
						Font1% = AALoadFont("GFX\font\TI\TI-83.ttf", Int(18 * (GraphicHeight / 1024.0)), 0,0,0)
						Font2% = AALoadFont("GFX\font\TI\TI-83.ttf", Int(58 * (GraphicHeight / 1024.0)), 0,0,0)
						Font3% = AALoadFont("GFX\font\DS-DIGI\DS-Digital.ttf", Int(22 * (GraphicHeight / 1024.0)), 0,0,0)
						Font4% = AALoadFont("GFX\font\DS-DIGI\DS-Digital.ttf", Int(60 * (GraphicHeight / 1024.0)), 0,0,0)
						Font5% = AALoadFont("GFX\font\Journal\Journal.ttf", Int(58 * (GraphicHeight / 1024.0)), 0,0,0)
		...
		If AchievementsMenu<=0 And OptionsMenu<=0 And QuitMSG<=0 Then
			If KillTimer >= 0 Then	
				...
				If (Not SelectedDifficulty\permaDeath) Then
					...
					Else
						...
						Color (100, 100, 100)
						AASetFont Font2
						AAText(x + (390*MenuScale) / 2, y + (60*MenuScale) / 2, "Reload", True, True)
					EndIf
					y = y + 75*MenuScale
			EndIf
				
				If DrawButton(x, y, 390*MenuScale, 60*MenuScale, "Unlockables") Then AchievementsMenu = 1
				y = y + 75*MenuScale
				If DrawButton(x, y, 390*MenuScale, 60*MenuScale, "Settings") Then OptionsMenu = 1
				y = y + 75*MenuScale
			Else
				y = y+104*MenuScale
				...
				Else
					...
					Color 50,50,50
					AAText(x + 185*MenuScale, y + 30*MenuScale, "Reload", True, True)
				EndIf
				If DrawButton(x, y + 80*MenuScale, 390*MenuScale, 60*MenuScale, "Escape to Menu") Then
					NullGame()
					MenuOpen = False
					MainMenuOpen = True
					MainMenuTab = 0
			...
			If KillTimer >= 0 And (Not MainMenuOpen)
				If DrawButton(x, y, 390*MenuScale, 60*MenuScale, "Escape") Then
					QuitMSG = 1
				EndIf
			EndIf
...
;	HideEntity OBJTunnel(6)
	...
	DoorFrameElevOBJ = LoadMesh_Strict("GFX\map\doorframeelev.x")
	HideEntity DoorFrameElevOBJ
	HideEntity DoorFrameOBJ
	
	HeavyDoorObj(0) = LoadMesh_Strict("GFX\map\heavydoornew.x")
	HideEntity HeavyDoorObj(0)
	HeavyDoorObj(1) = LoadMesh_Strict("GFX\map\heavydoornew.x")
