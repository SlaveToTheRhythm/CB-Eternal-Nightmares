...
Function CreateNPC.NPCs(NPCtype%, x#, y#, z#)
	...
	Select NPCtype
		...
		Case NPCtypeTSG
			;[Block]
			n\NVName = "tsg_placeholder"
			n\Collider = CreatePivot()
			EntityRadius n\Collider, 0.26
			EntityType n\Collider, HIT_PLAYER
			n\obj = LoadAnimMesh_Strict("GFX\npcs\tsg_placeholder.b3d")
			
			n\Speed = (GetINIFloat("DATA\NPCs.ini", "tsg_placeholder", "speed") / 100.0)
			
			temp# = (GetINIFloat("DATA\NPCs.ini", "tsg_placeholder", "scale") / 3.0)
			ScaleEntity n\obj, temp, temp, temp	
								...
								If Visible Then
									...
									If EntityInView(n\Collider, Camera) Then
										...
										If MilliSecs2() - n\LastSeen > 60000 Then 
											CurrCameraZoom = 120
											PlaySound_Strict(HorrorSFX(Rand(17,18)))
											n\LastSeen = MilliSecs2()
										EndIf
									EndIf
								Else
				...
				Select n\State
					Case 0
						...
						If dist<8.0 Then
							GiveAchievement(Achv096)
							;If n\Sound = 0 Then
							;	n\Sound = LoadSound_Strict("SFX\SCP\tsg_placeholder\096_1.ogg")
							;Else
							;	n\SoundChn = LoopSound2(n\Sound, n\SoundChn, Camera, n\Collider, 8.0, 1.0)
							;EndIf
							If n\SoundChn = 0
								n\SoundChn = StreamSound_Strict("SFX\SCP\tsg_placeholder\096_1.ogg",0)
								n\SoundChn_IsStream = True
							Else
								UpdateStreamSoundOrigin(n\SoundChn,Camera,n\Collider,8.0,1.0)
							EndIf
					...
					Case 4
						...
						If n\Target = Null Then 
							;If n\Sound = 0 Then
							;	n\Sound = LoadSound_Strict(ShyGuyScreamSFX(Rand(0,1)))
							;Else
							;	n\SoundChn = LoopSound2(n\Sound, n\SoundChn, Camera, n\Collider, 7.5, 1.0)
							;EndIf
							If n\SoundChn = 0
								n\SoundChn = StreamSound_Strict("SFX\SCP\tsg_placeholder\096S_1.ogg",0)
								n\SoundChn_IsStream = True
							Else
								UpdateStreamSoundOrigin(n\SoundChn,Camera,n\Collider,7.5,1.0)
							EndIf
							...
							;If n\Sound2 = 0 Then
							;	n\Sound2 = LoadSound_Strict("SFX\SCP\tsg_placeholder\096_4.ogg")
							;Else
							;	If n\SoundChn2 = 0 Then
							;		n\SoundChn2 = PlaySound_Strict (n\Sound2)
							;	Else
							...
							;EndIf
							If n\SoundChn2 = 0
								n\SoundChn2 = StreamSound_Strict("SFX\SCP\tsg_placeholder\096_4.ogg",0)
								n\SoundChn2_IsStream = 2
							Else
								SetStreamVolume_Strict(n\SoundChn2,Min(Max(8.0-dist,0.6),1.0)*SFXVolume#)
							EndIf
					...
					Case 1,2,3
						;[Block]
						;If n\Sound = 0 Then
						;	n\Sound = LoadSound_Strict("SFX\SCP\tsg_placeholder\096_2.ogg")
						;Else
						;	n\SoundChn = LoopSound2(n\Sound, n\SoundChn, Camera, n\Collider, 10.0, 1.0)
						;EndIf
						If n\SoundChn = 0
							n\SoundChn = StreamSound_Strict("SFX\SCP\tsg_placeholder\096_2.ogg",0)
							n\SoundChn_IsStream = True
						Else
							UpdateStreamSoundOrigin(n\SoundChn,Camera,n\Collider,10.0,1.0)
						EndIf
...
Function Console_SpawnNPC(c_input$, c_state$ = "")
	...
	Select c_input$ 
		...
		Case "tsg", "oldscp096", "tsg-placeholder"
			n.NPCs = CreateNPC(NPCtypeTSG, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			n\State = 5
			If (Curr096 = Null) Then Curr096 = n
			consoleMSG = "tsg_placeholder spawned."
